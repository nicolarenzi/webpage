# Pubblicazioni recenti


## 2026

<div class="publication-card">
    <div class="publication-image">
        <img src="/assets/swan.jpg" alt="Ode to Mosquito publication">
    </div>
    <div class="publication-content">
        <h3 class="publication-title">
            <a href="https://doi.org/10.14220/9783737019781.133" class="publication-link">
                An Ode to Culex: Listening with Mosquitoes, from Virgil to Sápmi
            </a>
        </h3>
        <div class="publication-venue">Listening to the Swan Song. More-than-Human Beings and Sounds in Literature and Culture Through the Ages (a cura di P. Kociumbas & J. Godlewicz-Adamiec)</div>
        <div class="publication-authors">Nicola Renzi</div>
        <div class="publication-year">2026</div>
        <div class="publication-tags">
            <span class="tag tag-safety">Zanzare</span><span class="tag tag-safety">Acustemologia</span><span class="tag tag-safety">Letteratura classica</span>
            <a href="https://doi.org/10.14220/9783737019781.133" class="tag tag-arxiv">DOI</a>
            <a href="https://www.academia.edu/167942985/An_Ode_to_Culex_Listening_with_Mosquitoes_from_Virgil_to_S%C3%A1pmi" class="tag tag-github">Academia.edu</a>
        </div>
    </div>
</div>


## 2025

<div class="publication-card">
    <div class="publication-image">
        <img src="/assets/mtm.png" alt="More-than-music publication">
    </div>
    <div class="publication-content">
        <h3 class="publication-title">
            <a href="http://hdl.handle.net/10138/593302" class="publication-link">
                More-than-music: Echosystems, acoustemologies and histories of listening from Sápmi
            </a>
        </h3>
        <div class="publication-venue">Dissertationes Universitatis Helsingiensis</div>
        <div class="publication-authors">Nicola Renzi</div>
        <div class="publication-year">2025</div>
        <div class="publication-tags">
            <span class="tag tag-safety">Antropologia ed Ecologia del Suono</span>
            <a href="http://hdl.handle.net/10138/593302" class="tag tag-arxiv">Archivio ufficiale</a>
            <a href="https://www.academia.edu/128358469/More_than_music_Echosystems_acoustemologies_and_histories_of_listening_from_S%C3%A1pmi" class="tag tag-github">Academia.edu</a>
        </div>
    </div>
</div>

## 2024

<div class="publication-card">
    <div class="publication-image">
        <img src="/assets/ES-SE.jpg" alt="Silence publication">
    </div>
    <div class="publication-content">
        <h3 class="publication-title">
            <a href="https://www.soundethnographies.it/vii-1-2-2024/etnografie-sonore-sound-ethnographies-n-vii-1-2024/renzi-2024/" class="publication-link">
                Rethinking Silence within Arctic Echosystems
            </a>
        </h3>
        <div class="publication-venue">Sound Ethnographies</div>
        <div class="publication-authors">Nicola Renzi (with stories by Inger-Mari Aikio, Mathis Eira and Ellen-Johanne Kvalsvik)</div>
        <div class="publication-year">2024</div>
        <div class="publication-tags">
            <span class="tag tag-safety">Silenzio</span>
            <span class="tag tag-safety">Ecologia Acustica</span>
            <span class="tag tag-safety">Estetica</span>
            <a href="https://www.soundethnographies.it/vii-1-2-2024/etnografie-sonore-sound-ethnographies-n-vii-1-2024/renzi-2024/" class="tag tag-arxiv">Sito della rivista</a>
            <a href="https://doi.org/10.5281/zenodo.17792187" class="tag tag-github">DOI</a>
        </div>
    </div>
</div>


<p>
  <a href="https://helsinki.academia.edu/NicolaRenzi" class="cactus-link" target="_blank" rel="noopener">
    Clicca per vedere tutte le pubblicazioni
  </a>
</p>

